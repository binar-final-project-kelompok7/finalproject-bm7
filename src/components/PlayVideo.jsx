...
function PlayVideo() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Replace axios.get with axios.post
        const response = await axios.post(
          `https://course-in-production.up.railway.app/api/v1/courses/${courseCode}`,
          // If your API requires data in the request body, pass it here
  ...
  return (
    <>
      {courseData && (
        ...
        <div className="video" style={{ backgroundColor: "#000000D9", width: "50%", height: "370px", display: "flex", justifyContent: "center", position: "relative" }}>
        <video
  ref={videoRef}
  src={courseData.link || videobg}
  autoPlay
  loop
  muted
