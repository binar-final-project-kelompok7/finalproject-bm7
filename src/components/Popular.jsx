...
const Popular = () => {
  ...
  return (
    <div className="container">
      <div className="row">
        <div className="col-6">
          <h4 className="text-start fw-bold" style={{marginTop:'40px'}}>Kursus Populer</h4>
        </div>
      ...
      <div className="navPopulars overflow-auto d-flex flex-row justify-content-center justify-content-md-start" style={{marginTop:'20px'}}>
        <div className="navPopular" style={{ width: "100px", marginBottom: '40px'}}>
          <button
            style={{ backgroundColor: selectedCategory === "All" ? "#6148FF" : "#E8F1FF", color: selectedCategory === "All" ? "#fff" : "#000",  fontWeight:'600' }}
            className="rounded-5"
  ...
  >
    <Link to={`/detail/${course.coursecode}`} key={`link-${course.coursecode}`} style={{color:'#000',textDecoration:'none'}}>
      ...
      <button
        ...
        style={{ backgroundColor: "#489CFF", width: "180px", height: "40px", marginBottom: "20px" }}
      >
        Beli {course.price}
