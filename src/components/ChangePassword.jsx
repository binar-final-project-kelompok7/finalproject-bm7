  ...
  const handleChangePassword = async (e) => {
    ...
    try {
      ...
      const patchResponse = await axios.put(
        `https://course-in-production.up.railway.app/api/v1/users/update-password/${usernameProfile}`,
        updatedPass,
        {
          headers: {
            Accept: "application/json",
    ...
    <div className='containerChangePass d-flex flex-column mx-auto pt-4'>
      <p></p>
        ...
        <form onSubmit={handleChangePassword} action="" className='d-flex flex-column text-start'>
            ...
            <button type='submit' className='changePassButton rounded-5 my-4 text-white'>Ubah Password</button>
        </form>
    </div>
  )
}
